...
class Direction {
  ...
  factory Direction.fromMapBoxMap(Map<String,dynamic> map){
    return Direction(
      routes: List<Route>.from(map['routes']?.map((x) => Route.fromMapBoxMap(x))),
...
class Route {
  ...
  factory Route.fromMapBoxMap(Map<String,dynamic> map){
    return Route(
  ...
  factory Route.fromMap(dynamic map) {
    return Route(
      duration: map['duration']?.toDouble() ?? 0.0,
      distance: map['distance']?.toDouble() ?? 0.0,
      geometryCordinates: List<LatLng>.from(map['geometryCordinates'].map( (x) => LatLng(x[0],x[1]))),
      instructions: List<RouteInstruction>.from(
          map['instructions']?.map((x) => RouteInstruction.fromMap(x))),
    );
  }
...
class RouteInstruction {
  ...
  factory RouteInstruction.fromMap(dynamic map) {
    return RouteInstruction(
